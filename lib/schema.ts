// Single source of truth for every option list in the questionnaire.
// The frontend form, the docx generator, and the analysis dashboard all
// import from here so labels and order always stay in sync with the
// original ATC document.
...
// Table 5 - weighting of components. Columns A-E carry the college's
// pre-set percentages; "OTHER" lets the respondent specify their own.
export const WEIGHTING_COLUMNS: Record<string, { lecture: number; tutorial: number; practical: number; visit: number; professional: number }> = {
...
// ---- Types matching prisma/schema.prisma's JSON-encoded columns ----
