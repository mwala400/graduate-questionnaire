...
export async function GET(_req: NextRequest, { params }: { params: { type: string } }) {
  ...
  for (const row of rows) {
    ...
    archive.append(buffer, { name: `${def.slug}-${row.id}.docx` });
  }
  archive.finalize();

  const zipBuffer = await streamToBuffer(passthrough);
  return new NextResponse(zipBuffer, {
    headers: { 'Content-Type': 'application/zip', 'Content-Disposition': `attachment; filename="${def.slug}-responses-docx.zip"` }
  });
}
